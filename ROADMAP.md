...
- New 2026-04-28 submit-boundary checkpoint: the latest `Invalid Anti-Bot Links` streak was not primarily an OCR tuning failure. Official FlareSolverr rejects `request.dom_submit`, then its `request.post` fallback corrupts ClaimCoin's space-separated `antibotlinks` field by percent-quoting it into a synthetic browser form. The working lane is the patched local FlareSolverr helper on `127.0.0.1:8195` with `request.dom_submit`. After switching local config to that helper and blocking the unsafe fallback for anti-bot payloads, two fresh live faucet claims succeeded with `12.00888 CCP has been added to your balance`; active telemetry at that checkpoint was `2 accepted / 6 total attempts`.
- New requests-first checkpoint: the preferred faucet lane is now HTTP-first again. The runner uses `curl_cffi` Chrome impersonation when available, detects Cloudflare challenge HTML, sends the current HTTP cookies into FlareSolverr when a bootstrap is needed, imports the solved cookies/user-agent back into the HTTP session, and uses FlareSolverr-rendered `/faucet` HTML only when the direct HTTP page lacks the reCAPTCHA v3 token. The final `/faucet/verify` submit is now HTTP-direct, not DOM-submit, and persists anti-bot telemetry. Live proof: `.venv/bin/python -m claimcoin_autoclaim.cli claim-once --config accounts.yaml` returned `ok=True`, `http_direct_submit=true`, and `12.00888 CCP has been added to your balance`; telemetry after that verified run was `5 accepted / 9 total attempts`.
- New antibot performance checkpoint: ClaimCoin now prefers the persistent antibot API at `127.0.0.1:8010` over the old per-claim subprocess core. The solver itself lives inside the `indra-api-hub` repo at `integrations/antibot-image-solver`; the ClaimCoin systemd unit now points to that real path instead of stale `tmp-gh`. Live API-provider proof: two immediate `claim-once` runs succeeded with `solver_provider=api`, wall times `14.03s` and `15.76s`, and success text `12.00888 CCP has been added to your balance`.
